...
%     indices = find(constVar.vy < 0 & (constVar.hd > 85 | constVar.hd < -85));
    ...
    logVar.probe = zeros(1, length(length(spkData.timestamps)));
    ...
    for tt = 1:length(startIdx)        
        ...
        elseif tt == length(startIdx)
            ...
            logVar.probe(startIdx(tt):endIdx(tt)) = probe(tt);
        end
    end
    ...
    constVar.cyclicY = constVar.cyclicY';
    ...
    all_variables.ylin = constVar.cyclicYLin;
    all_variables.y = constVar.cyclicY;
    ...
    constVariables.y = constVar.cyclicY';
    constVariables.ylin = constVar.cyclicYLin';
    constVariables.freq = constVar.freq;
    constVariables.relDistStop = constVar.relDistStop;
    % constVariables.lfp = constVar.lfp;
    
    save(fullfile(destination, strcat(extractAfter(destination, 'Final\'), ...
    ...
    save(fullfile(destination, strcat(extractAfter(destination, 'Final\'), '.labelsCEBRA.mat')), ...
        'x', 'y', 'direction', 'frequency', 'relDistStop', 'relDistStop_2d', 'toneTrial', 'probe', ...

...
function [resultsPGAM] = postprocessPGAM_newVars(varargin)

close all 
format long g

p = inputParser;
addParameter(p,'basepath','F:\PGAM\IZ43_220828_sess4\new_vars\tests\fullLicks_fullTypes_noHist',@isstr);
addParameter(p,'dataDir','I:\Videos\IZ43_220828_sess4',@isstr);
addParameter(p,'saveDir','F:\PGAM\IZ43_220828_sess4\new_vars\tests\fullLicks_fullTypes_noHist\postprocess',@isstr);
addParameter(p,'mapDir','Z:\Homes\zutshi01\Recordings\Auditory_Task\IZ43\Final\IZ43_220828_sess4\Maps',@isstr);parse(p,varargin{:});
...
for i = 1:length(pyramidal)
    ...
    if table2array(results(11, 10)) < 0.001
        tuned_licks(i, 1) = 1; 
    end
...
saveas(gcf, fullfile(saveDir, 'MI_licks_distribution'), 'fig');
saveas(gcf, fullfile(saveDir, 'MI_licks_distribution'), 'png');
close(figure(2));
...
saveas(gcf, fullfile(saveDir, 'MI_licks_heatmap'), 'fig');
saveas(gcf, fullfile(saveDir, 'MI_licks_heatmap'), 'png');
close(figure(3));
...
tuned_freq = zeros(length(pyramidal),1);
tuned_relDistStop = zeros(length(pyramidal),1);
...
resultsPGAM.tunedFreq = tuned_freq;
...
freqProp = length(find(tuned_freq == 1)) / length(pyramidal);
...
cells_y_freq = intersect(find(tuned_y == 1), find(tuned_freq == 1));
cells_relDistStop_freq = intersect(find(tuned_relDistStop == 1), find(tuned_freq == 1));
cells_licks_freq = intersect(find(tuned_licks == 1), find(tuned_freq == 1));
...
y_freq_prop = length(cells_y_freq) / length(pyramidal);
relDistStop_freq_prop = length(cells_relDistStop_freq) / length(pyramidal);
licks_freq_prop = length(cells_licks_freq) / length(pyramidal);
y_licks_relDistStop_freq_prop = length(cells_all) / length(pyramidal);

variables = [yProp, ylinProp, freqProp, relDistStopProp, lfpProp, licksProp, ...
    y_relDistStop_prop, licks_relDistStop_prop, y_licks_prop, ...
    y_freq_prop, relDistStop_freq_prop, licks_freq_prop, ...
    y_licks_relDistStop_freq_prop];
variable_names = ["y", "ylin", "freq", "relDistStop", "lfp", "licks", "y relDistStop", ...
    "licks relDistStop", "y licks", "y freq", "relDistStop freq", ...
    "licks freq", "y licks relDistStop freq"];
...
% y vs freq 
cells_y_freq = pyramidal(tuned_y == 1 & tuned_relDistStop == 1);

mi_y = zeros(length(cells_y_freq), 1);
mi_freq = zeros(length(cells_y_freq), 1);

tuned_freq_mi = zeros(length(cells_y_freq), 1);

for i = 1:length(cells_y_freq)
    if exist(fullfile(basepath, strcat('spatial_neuron_', ...
        string(cells_y_freq(i)), '_fit_k-fold.csv')))

        results = readtable(fullfile(basepath, strcat('spatial_neuron_', ...
        string(cells_y_freq(i)), '_fit_k-fold.csv')));

        mi_y(i, 1) = table2array(results(2, 11));
        mi_freq(i, 1) = table2array(results(3, 11));
...
mi_freq = normalize(mi_freq, "range");
mi_y = normalize(mi_y, "range");
...
resultsPGAM.tuned_freq_mi = cells_y_freq(tuned_freq_mi==1)';
...
% hold off
% 
% saveas(gcf, fullfile(saveDir, 'y_ylin_relDistStop_MI'), 'fig');
% saveas(gcf, fullfile(saveDir, 'y_ylin_relDistStop_MI'), 'png');
% 
...
% legend
% 
% saveas(gcf, fullfile(saveDir, 'y_ylin_relDistStop_scatter'), 'fig');
% saveas(gcf, fullfile(saveDir, 'y_ylin_relDistStop_scatter'), 'png');
...
resultsPGAM.tuned_freq_relDistStop = pyramidal(tuned_freq == 1 & tuned_relDistStop == 1)';
resultsPGAM.tuned_nofreq_relDistStop = pyramidal(tuned_freq == 0 & tuned_relDistStop == 1)';
resultsPGAM.tuned_licks_freq = pyramidal(tuned_freq == 1 & tuned_licks == 1)';
resultsPGAM.tuned_nolicks_freq = pyramidal(tuned_freq == 1 & tuned_licks == 0)';
resultsPGAM.tuned_licks = pyramidal(tuned_licks == 1)';
resultsPGAM.tuned_freq = pyramidal(tuned_freq == 1)';
...
for i = 1:length(pyramidal)
    ...
    if table2array(results(11, 18)) > 0
...
% saveas(gcf, fullfile(saveDir, 'maps_tuned_relDistStop_licks'), 'fig');
% 
...
% saveas(gcf, fullfile(saveDir, 'maps_tuned_relDistStop_noLicks'), 'fig');

%% Save results
if exist(fullfile(saveDir, strcat(extractAfter(dataDir, 'Videos\'), ...
    '.postprocessPGAM.mat')))
    delete(fullfile(saveDir, strcat(extractAfter(dataDir, 'Videos\'), ...
    '.postprocessPGAM.mat')));
end

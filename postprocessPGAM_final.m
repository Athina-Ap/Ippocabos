...
addParameter(p,'fits','I:\April 2023 GAM fits\results\new_vars2',@isstr);
...
for s = 1:length(sessions)
    ...
    if ~isempty(dir([baseDir filesep '*.cell_metrics.cellinfo.mat']))
        ...
        copyfile(fullfile(baseDir,file.name), dataDir);
        load(fullfile(dataDir, file.name));
    end
    ...
    licksProp = length(find(tuned_licks == 1)) / length(pyramidal);
    ...
    % All neurons
    yProp_all = length(find(tuned_y_all == 1)) / length(neurons);
    ylinProp_all = length(find(tuned_ylin_all == 1)) / length(neurons);
    relDistStopProp_all = length(find(tuned_relDistStop_all == 1)) / length(neurons);
    licksProp_all = length(find(tuned_licks_all == 1)) / length(neurons);
    ...
    y_relDistStop_prop = length(tuned_y_relDistStop) / length(pyramidal);
    licks_relDistStop_prop = length(tuned_licks_relDistStop) / length(pyramidal);
    y_licks_prop = length(tuned_y_licks) / length(pyramidal);
    y_licks_relDistStop_prop = length(tuned_y_licks_relDistStop) / length(pyramidal);
    ...
    y_relDistStop_prop_all = length(tuned_y_relDistStop_all) / length(neurons);
    licks_relDistStop_prop_all = length(tuned_licks_relDistStop_all) / length(neurons);
    y_licks_prop_all = length(tuned_y_licks_all) / length(neurons);
    y_licks_relDistStop_prop_all = length(tuned_y_licks_relDistStop_all) / length(neurons);

function [placeFieldStats] = bz_findPlaceFields1D_AA(varargin)
...
if isempty(firingMaps)
    if exist('MergePoints')
        firingMaps = load([basepath filesep basename '.firingMapsCond.cellinfo.mat']);
        firingMaps = firingMaps.firingMapsCond;
        sizeMaze = length(firingMaps{1}.rateMaps{1}{1});
    else
        firingMaps = load([basepath filesep basename '.firingMapsTrial.cellinfo.mat']);
...
%% Two session
if exist('MergePoints')
    for cond = 1:length(firingMaps)
        % Find place fields
        [placeFieldStats{cond}, mapStats{cond}] = getPlaceFields(firingMaps{cond}, threshold, ...
            minSize, maxSize, minPeak, minPeak2nd, sepEdge, sizeMaze, verbose, saveMat);
        
        % Make plots
        labels_cond{1} = strcat([labels{1}, ' ', conditions{cond}]);
        labels_cond{2} = strcat([labels{2}, ' ', conditions{cond}]);
        plotPlaceFields(firingMaps{cond}, mapStats{cond}, labels_cond, basepath)    
    end
    
%% One session
else
    % Find place fields
    [placeFieldStats,mapStats] = getPlaceFields(firingMaps, threshold, minSize, ...
        maxSize, minPeak, minPeak2nd, sepEdge, sizeMaze, verbose, saveMat);    

    % Make plots
    plotPlaceFields(firingMaps, mapStats, labels, basepath)

end

%% Save output
if saveMat
   save([basepath,filesep,basename '.placeFields.cellinfo.mat'],'placeFieldStats'); 
end
...
% ------------------------------- Helper functions -------------------------------
    ...
    %% (1) FieldBoundaries (circumscribed rectangle)
    ...
    %% (2) plotPlaceFields 
    function plotPlaceFields(firingMaps, mapStats, labels, basepath)
    ...
    for c = 1:length(firingMaps.rateMaps{1})
        ...
        saveas(gcf,[basepath,filesep,'newPCs',filesep ,'map_' labels{c} '.png'],'png');
    end
    end

    %% (3) getPlaceFields
    function [placeFieldStats, mapStats] = getPlaceFields(firingMaps, threshold, minSize, ...
        maxSize, minPeak, minPeak2nd, sepEdge, sizeMaze, verbose, saveMat)
    ...
    placeFieldStats.params.start = firingMaps.start;
